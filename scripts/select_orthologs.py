...
def parse_header(header: str) -> Tuple[str, str]:
    """Extract (species_id, full_header) from FASTA header.
    Format: >G01_WP_12345... -> ('G01', 'G01_WP_12345...')
    """
    parts = header.split('_', 1)
    if len(parts) >= 2:
        return parts[0], header

    # Single token without underscore (whole thing as species ID)
    if parts[0]:
        return parts[0], header
    ...
    try:
        ...
        if count > 0:
            ...
            try:
                ...
                print(f"  BLAST fallback for cluster: {e}", file=sys.stderr)     
    finally:
...
def process_cluster_job(args: Tuple) -> Tuple[str, Any]:
    ...
    # Filter to proteins actually present in SEQ_DB
    valid_members: Dict[str, List[str]] = {}
    for sp, prots in members_dict.items():
        valid_prots = [p for p in prots if p in SEQ_DB]
        if valid_prots:
            valid_members[sp] = valid_prots

    # Singletons: cluster has only 1 protein total
    total_proteins = sum(len(prots) for prots in valid_members.values())
    if total_proteins <= 1:
        return ("SINGLETON", None)

    # Min Species Check (Ingroup only)
    ingroup_species = [sp for sp in valid_members.keys() if sp not in outgroups]
    ...
    if mode == "paralogs":
        for sp, prots in valid_members.items():
        ...
        # SCO mode: select one representative per species
        single_copy_species = {sp: prots[0] for sp, prots in valid_members.items()
                               if len(prots) == 1}
        single_copy_proteins = list(single_copy_species.values())
...
def main():
    ...
    outgroups = set(snakemake.params.outgroups) if hasattr(snakemake.params, "outgroups") else set()
    mode = str(snakemake.params.mode) if hasattr(snakemake.params, "mode") else "sco"
    ...
    with open(cluster_tsv, 'r') as f:
        for line in f:
            ...
            # Deduplicate: MMseqs2 can emit duplicate rows
            if (rep, member) in seen_members:
                continue

            seen_members.add((rep, member))
    ...
    print(f"[INFO]: Detected {TOTAL_INGROUP_COUNT} unique ingroup species in input data.", file=sys.stderr)
    ...
    with Pool(processes=threads) as pool:
        for result in pool.imap_unordered(process_cluster_job, tasks, chunksize=50):
            ...
            if status == "OK":
                ...
                print(f"[INFO]: Cluster {rep} is a singleton.", file=sys.stderr)
                stats["Singletons"] += 1
            elif status == "LOW_SPECIES":
                print(f"[INFO]: Cluster {rep} has low species coverage.", file=sys.stderr)
                stats["Low Species Coverage"] += 1
            elif status == "NO_ANCHOR":
                print(f"[INFO]: Cluster {rep} has no anchors (SCO only).", file=sys.stderr)
                stats["No Anchors (SCO only)"] += 1

    print(f"[INFO]: Writing detailed report to {stats_file}.", file=sys.stderr)

    with open(stats_file, "w") as f:
        f.write(f"SELECTION REPORT\n")
        f.write(f"=========================\n")
...
if __name__ == "__main__":
    if _SNAKEMAKE_MODE:
        ...
        print("[INFO]: For testing, import the module functions directly.", file=sys.stderr)
        sys.exit(1)


# When loaded by Snakemake
if _SNAKEMAKE_MODE and __name__ != "__main__":
    main()
