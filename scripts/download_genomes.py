...
def run_datasets_cli(identifier: str, 
    ...
    try:
        ...
        print(f"[WARNING]: {sample_id} with unexpected error processing {identifier}: {e}", file=sys.stderr)
        return False
    finally:
...
# Snakemake script: entry point (no __name__ guard — Snakemake uses exec())
