...
sed 's/:[0-9.eE\-]*//g' "$CLEAN_INPUT" > "${CLEAN_INPUT}.tmp" && mv "${CLEAN_INPUT}.tmp" "$CLEAN_INPUT"
sed 's/)[0-9.]*/)/g' "$CLEAN_INPUT" > "${CLEAN_INPUT}.tmp" && mv "${CLEAN_INPUT}.tmp" "$CLEAN_INPUT"
sed 's/_p[0-9]*//g' "$CLEAN_INPUT" > "${CLEAN_INPUT}.tmp" && mv "${CLEAN_INPUT}.tmp" "$CLEAN_INPUT"
...
if [ -n "$GENERATED_FILE" ] && [ -s "$GENERATED_FILE" ]; then
    echo "[INFO]: Found output file: $GENERATED_FILE"
